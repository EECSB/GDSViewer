...
```mermaid
flowchart TD
    Host["wwwroot/index.html<br>the base href, then the import map, then six scripts"]
    Host --> Boot["blazor.webassembly.js"]
    Host --> Beside["viewGeometry.js, sidebars.js, StorageInterop.js<br>the first of those is pure, and the one Node can test"]
...
GDSViewer/
  ...
  wwwroot/
    ...
    js/                         # ThreeInterop, MonacoInterop, JavaScriptInterOp,
                                #   StorageInterop, sidebars, viewGeometry
...
exactly until the next nudge of that slider. `SetStackingOffsets` skips a layer that has been placed, and
the step still counts past it, so the layers around one real height stay where they would have been rather
than closing up the gap it left.
...
**The cost is the old reasoning's, and it is real**: a pin or a label purpose now floats a step above the
geometry it annotates, and the stack is as tall as the file has purposes — 46 planes rather than 21 across the
bundled corpus. A layermap with real heights in it is what gets a physical stack; this is what a file without
one gets, and pulling it open is what the slider is for.
...
for something most people never look at. It is deferred to `drawSelectedPreview`, which runs when a popup
opens. Still once per file rather than once per keystroke, which is why it had been hoisted out of the
...
what a session puts back, and the 2D editor — measuring, picking shapes out,
...
- **No layer names ship with the app, by choice.** Layers can be named — typed on the row or loaded from a
  ...
  so an upload, a history entry and a page's injected example all get nothing. Names already in hand win, and
  so does `layermap=`. And it is deliberately **not** written into the field that carries names from one file
  open to the next, because that field reaches uploads too: an assumption should not follow the visitor onto a
